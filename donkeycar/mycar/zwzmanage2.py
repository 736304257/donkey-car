...
bz_state = 1
    #目标识别
class VideoStream:
    def __init__(self,resolution=(720,1280),framerate=30):
        # Initialize the PiCamera and the camera image stream
        self.stream = cv2.VideoCapture(0)
        ret = self.stream.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
        ret = self.stream.set(3,resolution[0])#调整视频宽
        ret = self.stream.set(4,resolution[1])#调整视频高
    ...
    # Start the thread that reads frames from the video stream
        Thread(target=self.update,args1=()).start()
        return self
        ...
        def run(self, mode,
            ...
            if distance < 55 or bz_state:
                ...
                if distance > 100:
                    bz_state = 0
                    print('避障调整中 ...')
            elif distance >= 55 and bz_state == 0:
                user_throttle = 0.75
                user_angle = -0.3
            print('角度：', user_angle, '; 油门：', user_throttle)
            if mode == 'user':
