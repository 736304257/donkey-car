...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    ...
    class DriveMode:
        ...
        def run(self, mode,
            ...
            #print('1:',distance)
            # pos = cam.update()
            # print("nbaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaawz",pos)
            # pos = cam.run()
            # print(pos)
            # pos = cam.pos_get()
            # print(pos)
            # print(dk.parts.camera.outputxmin)
            with open("/home/pi/projects/donkeycar/parts/pos_data.txt", "rb") as f:
                try:
                    ...
                    return None
            #print("nbaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaawz",(outputxmin,outputymin),(outputxmax,outputymax))
            p=[outputxmin,outputymin,outputxmax,outputymax]
            if music_on == 1 and music_ing==0:
                pygame.mixer.music.load("music1.mp3")
                pygame.mixer.music.play(-1)
                music_ing = 1
            if music_on == 0 and music_ing == 1:
            ...
            if p[0]<0:#若未检测到行人
                ...
                if distance >= 30:
                    ...
                    user_throttle = 0.0
                    user_angle = -0.3
                # return user_angle if user_angle else 0.0, user_throttle * cfg.AI_THROTTLE_MULT if user_throttle else 0.0
            if mode == 'user':
